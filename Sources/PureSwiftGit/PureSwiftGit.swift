...
public struct TreeItem: Hashable {
    ...
    var data: Data {
        "\(mode) \(name)".data(using: .utf8)! + [0] + hash.hexBytes
    }
}
...
extension String {
    var hexBytes: Data {
        ...
        while !remainder.isEmpty {
            let digits = remainder.prefix(2)
            remainder.removeFirst(2)
            let byte = UInt8(digits, radix: 16)!
            result.append(byte)
        }
        return result
    }
...
public struct Commit: Hashable {
    ...
    var string: String {
        metadata.map { kv in
            "\(kv.key) \(kv.value.replacingOccurrences(of: "\n", with: "\n "))"
        }.joined(separator: "\n")
        + "\n\n"
        + message
    }
...
extension Object {
    var data: Data {
        ...
        case .commit(let commit):
            ...
            return "commit \(data.count)".data(using: .utf8)! + [0] + data
        case .tree(let array):
            let data = Data(array.map { $0.data }.joined())
            return "tree \(data.count)".data(using: .utf8)! + [0] + data
        }
    }
}
